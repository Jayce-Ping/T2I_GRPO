...
def flux_step(
    ...
    prev_sample_mean = latents + dsigma * model_output # Less noise, towards clear image

    # pred_original_sample corresponds to 'original clear image'.
    pred_original_sample = latents - sigma * model_output

    delta_t = sigma - sigmas[index + 1] # > 0
    std_dev_t = eta * math.sqrt(delta_t) # standard deviation of noise

    if sde_solver:
        # Use sde solver to revise
        score_estimate = -(latents - pred_original_sample * (1 - sigma)) / sigma**2
        log_term = -0.5 * eta**2 * score_estimate
        prev_sample_mean = prev_sample_mean + log_term * dsigma

    if grpo and prev_sample is None:
        # If run grpo, add noise for stochastic samples
...
def train_one_step(
    ...
    # If use group, caculate advantages for each group locally
    # If not, calculate advantages globally
    if args.use_group:
    ...
    for i,sample in list(enumerate(samples_batched_list)):
        for _ in range(train_timesteps):
            ...
            # ratio = new_policies / old_policies > 0
            ...
            # bascially, objective = advantage * ratio, but with clipping
            # to maximize the objective, we minimize the negative of it (torch optimize minimize the loss as default)
